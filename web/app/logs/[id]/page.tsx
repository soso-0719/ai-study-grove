...
export default function LogDetailPage({ params }: PageProps) {
    ...
    useEffect(function () {
        async function load() {
            ...
            const id = p.id;
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/study-logs/${id}`, {
                headers: authHeaders()
            });
            if (!res.ok) {
                setError("ログが見つかりません");
            ...
            const feedbackRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/ai-feedbacks/${id}`, {
                headers: authHeaders()
            });
            if (feedbackRes.ok) {
    ...
    async function handleDelete() {
        if (!log) return;
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/study-logs/${log.id}`, {
            method: "DELETE",
            headers: authHeaders()
        });
        if (res.ok) {
